...
module.exports = function xresilient(fn, opts) {
  ...
  function work() {
    ...
    const source = (store.source = store.fn({
      bytesRead: store.bytesRead,
      oldStream: store.source,
    ...
    function handleErr(err) {
      ...
      else store.xtream.emit('retry', {retryCount: store.retryCount, bytesRead: store.bytesRead, lastErr: err}), work();
    }
