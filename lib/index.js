...
module.exports = function xresilient(fn, opts) {
  ...
  const store = {fn, bytesRead: 0, source: null, retries: 5, retryCount: 0};
  opts = opts || {};
  ...
  function work(retrySlice) {
    ...
    function onErr(err) {
      ...
      // eslint-disable-next-line no-underscore-dangle
      if (!source._readableState.ended && source._readableState.length) store.xtream.once('drain', onceDrain.bind(null, err));
      else onceDrain(err);
    }
    function onEnd() {
      source.unpipe(store.xtream);
