...
module.exports = function xresilient(fn, opts) {
  ...
  function work(oldStream) {
    // eslint-disable-next-line no-underscore-dangle
    const _source = store.fn({
    ...
    function onErr(err) {
      ...
      else store.xtream.emit('retry', {retryCount: store.retryCount, bytesRead: store.bytesRead, lastErr: err}), work(_source);
    }
  ...
  store.xtream.setRetries = retries => {
    ...
    if (retries < store.retryCount) return false;
    store.retries = retries;
    return true;
  };
  return work(), store.xtream;
};
