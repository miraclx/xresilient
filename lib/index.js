...
module.exports = function xresilient(fn, opts) {
  ...
  async function buildUnderlayer(retrySlice) {
    ...
    function ondrain(err, preKilled) {
      if (preKilled) store.destroyer(_source);
    ...
    function onerr(err) {
      cleanup();
      if (!(source._readableState.ended || source.destroyed) && source._readableState.length)
        source.once('drain', ondrain.bind(null, err));
      else ondrain(err);
    }
    ...
    if (_source._readableState.length === 0) {
      if (_source._readableState && _source._readableState.ended) source.push(null);
      if (_source._readableState && (_source._readableState.destroyed || _source._readableState.errorEmitted))
        ondrain(undefined, true);
    }
  }
  ...
  function containedUnderlayer(xdataSlice) {
    ...
    if (!store.retries || store.retryCount === store.retries) self.destroy(xdataSlice.lastErr);
    else {
